...
import { taxSchema } from "@deeppling/shared";
...
export async function POST(
  ...
  try {
    ...
    const payload = parseBody(taxSchema, await request.json());
    const actor = request.headers.get("x-actor-email") ?? `invite:${token.slice(0, 8)}`;
    const employee = services.onboarding.updateTax(token, actor, payload);
