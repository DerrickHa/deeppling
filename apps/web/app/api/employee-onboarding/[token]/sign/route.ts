...
import { signSchema } from "@deeppling/shared";
...
export async function POST(
  ...
  try {
    ...
    const payload = parseBody(signSchema, await request.json());
    const actor = request.headers.get("x-actor-email") ?? `invite:${token.slice(0, 8)}`;
    const employee = services.onboarding.signDocuments(token, actor, payload);
