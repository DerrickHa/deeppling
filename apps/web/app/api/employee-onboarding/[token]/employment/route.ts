...
import { employmentSchema } from "@deeppling/shared";
...
export async function POST(
  ...
  try {
    ...
    const payload = parseBody(employmentSchema, await request.json());
    const actor = request.headers.get("x-actor-email") ?? `invite:${token.slice(0, 8)}`;
    const employee = services.onboarding.updateEmployment(token, actor, payload);
